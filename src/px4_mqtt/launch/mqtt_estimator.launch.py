...
"""
Launch file for MQTT Estimator node.
...
def launch_setup(context):
    ...
    install_config_file = os.path.join(pkg_share, 'config', 'mqtt_estimator.yaml')
    ...
    if 'install' in pkg_share:
        ...
        source_config_file = os.path.join(src_dir, 'config', 'mqtt_estimator.yaml')
    ...
    mqtt_estimator_node = Node(
        package='px4_mqtt',
        executable='mqtt_estimator',
        name='mqtt_estimator',
        output='screen',
        parameters=parameters
    )

    return [mqtt_estimator_node]
