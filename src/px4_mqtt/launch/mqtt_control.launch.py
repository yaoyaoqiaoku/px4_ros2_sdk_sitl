...
"""
Launch file for MQTT Control node.
...
def launch_setup(context):
    ...
    install_config_file = os.path.join(pkg_share, 'config', 'mqtt_control.yaml')
    ...
    if 'install' in pkg_share:
        ...
        source_config_file = os.path.join(src_dir, 'config', 'mqtt_control.yaml')
    ...
    mqtt_control_node = Node(
        package='px4_mqtt',
        executable='mqtt_control',
        name='mqtt_control',
        output='screen',
        parameters=parameters
    )

    return [mqtt_control_node]
