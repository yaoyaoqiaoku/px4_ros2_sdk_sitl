...
"""
MQTTHome位置控制节点的启动文件。
订阅MQTT代理服务器的Home位置控制指令,并将指令发布到ROS2系统中。
"""
...
def generate_launch_description():
    ...
    mqtt_home_topic_arg = DeclareLaunchArgument(
        'mqtt_home_topic',
        default_value='',
        description='MQTT Home位置话题前缀(若提供则覆盖YAML配置文件中的对应值)'
    )
...
def launch_setup(context):
    ...
    install_config_file = os.path.join(pkg_share, 'config', 'mqtt_home_control.yaml')
    ...
    if 'install' in pkg_share:
        ...
        source_config_file = os.path.join(src_dir, 'config', 'mqtt_home_control.yaml')
    ...
    mqtt_home_topic = LaunchConfiguration('mqtt_home_topic').perform(context)
    ...
    if mqtt_home_topic and mqtt_home_topic.strip():
        override_params['mqtt_home_topic'] = mqtt_home_topic
    ...
    mqtt_home_node = Node(
        ...
        executable='mqtt_home_control',  # 可执行文件名称
        name='mqtt_home_control',   # 节点名称
