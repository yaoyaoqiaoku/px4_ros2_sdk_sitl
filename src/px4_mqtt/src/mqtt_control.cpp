...
MQTTControl::MQTTControl() : Node("mqtt_control")
{
    ...
    this->declare_parameter<std::string>("mqtt_broker", "tcp://162.14.193.2:1883");
...
void MQTTControl::setup_publishers()
{
    ...
    vehicle_local_position_sub_ = this->create_subscription<px4_msgs::msg::VehicleLocalPosition>(
        ...
        std::bind(&MQTTControl::vehicle_local_position_callback, this, std::placeholders::_1));

    vehicle_status_sub_ = this->create_subscription<px4_msgs::msg::VehicleStatus>(
        "/fmu/out/vehicle_status", qos,
        std::bind(&MQTTControl::vehicle_status_callback, this, std::placeholders::_1));
}
...
void MQTTControl::handle_command(const json& cmd)
{
    ...
    if (command == "arm") {
    ...
    } else if (command == "velocity" || command == "velocity_ned") {
    ...
    } else if (command == "velocity_body") {
        if (!cmd.contains("vx") || !cmd.contains("vy") || !cmd.contains("vz")) {
            RCLCPP_WARN(this->get_logger(), "Velocity_body command missing vx, vy, or vz");
            return;
        }
        float vx = cmd["vx"].get<float>();
        float vy = cmd["vy"].get<float>();
        float vz = cmd["vz"].get<float>();
        float yaw_speed = cmd.value("yaw_speed", 0.0f);

        // Directly publish; PX4 control node will handle safe OFFBOARD entry
        publish_body_velocity_setpoint(vx, vy, vz, yaw_speed);
        ...
        try {
            ...
            if (idx >= 1 && idx <= 8) {
                float sub = static_cast<float>(10 + idx);
...
void MQTTControl::vehicle_local_position_callback(const px4_msgs::msg::VehicleLocalPosition::UniquePtr msg)
{
    vehicle_local_position_ = *msg;
    vehicle_local_position_received_ = true;
}

void MQTTControl::vehicle_status_callback(const px4_msgs::msg::VehicleStatus::UniquePtr msg)
{
    vehicle_status_ = *msg;
    vehicle_status_received_ = true;

    bool is_offboard = (vehicle_status_.nav_state == 14);
    vehicle_in_offboard_ = is_offboard;

    static bool last_offboard = false;
    if (vehicle_in_offboard_ && !last_offboard) {
        RCLCPP_INFO(this->get_logger(), "Detected OFFBOARD mode (nav_state=14)");
    } else if (!vehicle_in_offboard_ && last_offboard) {
        RCLCPP_INFO(this->get_logger(), "Exited OFFBOARD mode");
    }
    last_offboard = vehicle_in_offboard_;
}
...
void MQTTControl::publish_body_velocity_setpoint(float vx_b, float vy_b, float vz_b, float yaw_speed)
{
    float yaw = 0.0f;
    if (vehicle_local_position_received_) {
        yaw = vehicle_local_position_.heading;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Publishing body->NED velocity via MQTT: body=[%.2f, %.2f, %.2f], ned=[%.2f, %.2f, %.2f], yawspeed=%.2f",
                vx_b, vy_b, vz_b, vx_n, vy_n, vz_n, yaw_speed);
