...
def generate_launch_description():
    ...
    # 使用包内的 params.yaml（避免硬编码绝对路径）
    params_path = PathJoinSubstitution([FindPackageShare('precision_land'), 'cfg', 'params.yaml'])

    return LaunchDescription([
        enable_viz_arg,

        # 1. 桥接Gazebo的/camera到ROS2的/camera（核心修改） sudo apt install ros-humble-ros-gzharmonic桥接包一定安装正确
        Node(
